...
//               <Link to={STORE_ROUTES.ROOT} className="hidden sm:flex items-center gap-2 text-sm font-semibold text-primary-500 border border-primary-300 rounded-full px-4 py-2 hover:bg-primary-50 transition-colors">
//                 Continue Shopping
//               </Link>
...
//                   return (
//                     <div key={item.id} className={`bg-white rounded-2xl border p-4 flex items-start gap-4 ${isOverStock ? 'border-amber-300' : 'border-gray-100'}`}>
...
//                             className="w-8 h-8 rounded-full border-2 border-gray-200 flex items-center justify-center text-gray-500 hover:border-gray-400 disabled:opacity-40 transition-colors">−</button>
...
//                         <button onClick={() => handleEdit(item.id)} className="text-primary-400 hover:text-primary-600 transition-colors p-1" title="Edit item"><Pencil size={16} /></button>
//                         <button onClick={() => setConfirmDelete(item.id)} className="text-red-400 hover:text-red-600 transition-colors p-1" title="Remove item"><Trash2 size={16} /></button>
//                       </div>
//                     </div>
//                   );
//                 })
...
//                     items.length === 0 || hasOverstockedItems ? 'pointer-events-none opacity-50' : ''
//                   }`}
//                 >
//                   Checkout
//                 </Link>
    ...
    return (
      <div className="flex items-center gap-2 mt-3">
        <button
          onClick={() => handleQtyChange(item.id, item.quantity - 1, stockCap)}
          disabled={item.quantity <= 1}
          className="w-8 h-8 rounded-full border-2 border-gray-200 flex items-center justify-center text-gray-500 hover:border-gray-400 disabled:opacity-40 disabled:cursor-not-allowed disabled:hover:border-gray-200 transition-colors">−</button>
        <input
          type="number"
          min={1}
          max={stockCap}
        ...
        <button
          onClick={() => handleQtyChange(item.id, item.quantity + 1, stockCap)}
          disabled={item.quantity >= stockCap}
          className="w-8 h-8 rounded-full border-2 border-gray-200 flex items-center justify-center text-gray-500 hover:border-gray-400 disabled:opacity-40 hover:border-gray-400 disabled:cursor-not-allowed disabled:hover:border-gray-200 transition-colors">+</button>
      </div>
    );
  }
                    ...
                    <div key={item.id} className={`bg-white rounded-2xl border p-4 flex items-start gap-4 ${isOverStock ? 'border-amber-300' : 'border-gray-100'}`}>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {isOverStock && (
                          ...
                          <CartQtyStepper item={item} stockCap={stockCap} />
                      </div>
                      <div className="flex flex-col items-end gap-2 shrink-0">
                        <button onClick={() => handleEdit(item.id)} className="text-primary-400 hover:text-primary-600 transition-colors p-1" title="Edit item"><Pencil size={16} /></button>
                        <button onClick={() => setConfirmDelete(item.id)} className="text-red-400 hover:text-red-600 transition-colors p-1" title="Remove item"><Trash2 size={16} /></button>
