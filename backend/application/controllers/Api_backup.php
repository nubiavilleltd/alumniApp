 ...
 private function checkJWT()
	    {
	        ...
	        // 3. Fallback: accept jwt field in POST/form-data (for multipart/form-data requests)
	        if (!$token) {
...
public function get_chapters()
{
    ...
    public function create_announcement()
    {
        ...
        $data = [
            ...
            'title'     => $title,
            'content'   => $content,
            'image_url' => !empty($uploaded_files) ? json_encode($uploaded_files, JSON_UNESCAPED_SLASHES) : null,
            'created_at' => date('Y-m-d H:i:s')
        ];
        ...
        $response = [
            ...
            'data' => array_merge(['id' => $market_id], $data)
        ];
        $this->output->set_status_header(200);
        header('Content-Type: application/json');
        echo json_encode($response);
    ...
    public function manage_announcement()
    {
        ...
        if (strpos($contentType, 'application/json') !== false) {
            $body   = file_get_contents("php://input");
            $object = json_decode($body, true);
            log_message('error', "Manage Announcement Input: " . var_export($object, true));
        } else {
            $object = $this->input->post();
            log_message('error', "Manage Announcement Input2: " . var_export($object, true));
        }
        ...
        $function_type = isset($object['function_type']) ? strtolower(trim($object['function_type'])) : '';
        $id = isset($object['id']) ? trim($object['id']) : null;
        ...
        if ($function_type === 'update') {
            ...
            if (!empty($object['title']))   $update_data['title']   = $object['title'];
            if (!empty($object['content'])) $update_data['content'] = $object['content'];
            ...
            // === Merge with existing images if any ===
            if (!empty($uploaded_files)) {
                $existing = $this->db->select('image_url')->from('announcements')->where('id', $id)->get()->row();

                $existingImages = [];
                if ($existing && !empty($existing->image_url)) {
                    $decoded = json_decode(stripslashes($existing->image_url), true);
                    if (is_array($decoded)) $existingImages = $decoded;
                }

                $mergedImages = array_merge($existingImages, $uploaded_files);
                $update_data['image_url'] = json_encode($mergedImages, JSON_UNESCAPED_SLASHES);
            }
            ...
            $this->db->where('id', $id)->update('announcements', ['is_deleted' => 1]);
    ...
    public function get_announcements()
    {
        ...
        foreach ($result as &$row) {
            $row['image_url'] = $row['image_url']
                ? json_decode($row['image_url'], true) // ✅ decode safely
                : [];
        }
...
public function get_users_by_action()
{
   ...
   protected function sendUserMail($email, $subject, $fullname, $action, $actor = 'admin')
   {
       ...
       $mail->setFrom('appnotice@nubiaville.com', 'Alumni Portal Notifier');
       $mail->addAddress($email, $fullname);
       $mail->addBCC('appnotice@nubiaville.com');
    ...
    protected function sendStaffMail($email, $subject, $fullname, $password, $user_id): void
    {
        ...
        ;
        $this->email->from('appnotice@nubiaville.com', 'Estate Management Notifier');
...
public function get_listings()
{
    ...
    public function verify_email()
    {
        ...
        $this->_sendAdminNewAccountNotification($user['fullname'], $user['email'], 'New FGGC Alumni Account Pending Approval');
    ...
    private function sendVerifyEmail($email, $fullname, $verify_token)
    {
        ...
        $body = $this->load->view('auth/email/template', $data, TRUE);
        $this->email->clear();
        $this->email->from('appnotice@nubiaville.com', 'Alumni Portal');
        $this->email->to($email);
        $this->email->bcc('appnotice@nubiaville.com');
        $this->email->set_newline("\r\n");
        $this->email->set_crlf("\r\n");
