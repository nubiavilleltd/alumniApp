...
export function RegisterForm() {
  ...
  // ── Filter vouchers when graduation year changes ───────────────────────────
  useEffect(() => {
    if (graduationYear) {
      ...
      // Clear selected voucher if it's no longer valid for the new year
      const currentVoucherId = detailForm.getValues('voucherId');
      if (currentVoucherId) {
        const isValid = filtered.some((v) => String(v.id) === currentVoucherId);
        if (!isValid) {
  ...
  }, [graduationYear, allVouchers, detailForm]);
  ...
  // ── Restore flow from sessionStorage ───────────────────────────────────────
  useEffect(() => {
    const saved = loadFlow();
    if (saved) {
      setFlow(saved);
      ...
      {flow.step === 'details' && (
        <form className="space-y-4" onSubmit={submitDetails}>
          ...
          <SelectInput
            ...
            onChange={(e) => {
              detailForm.setValue('graduationYear', parseInt(e.target.value), {
                shouldValidate: true,
                shouldDirty: true,
              });
            }}
          ...
          {/* <SelectInput
            ...
            hint="Select a fellow alum who will vouch for you"
            {...detailForm.register('voucherId')}
          /> */}
          ...
          <SelectInput
            ...
            onChange={(e) => {
              detailForm.setValue('voucherId', e.target.value, {
                shouldValidate: true,
                shouldDirty: true,
              });
            }}
