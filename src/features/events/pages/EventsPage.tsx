...
// import type { RegisterEventModalEvent } from './RegisterEventModal';
...
function EventCard({ event, isPast, onRegister }: { event: AlumnaeEvent; isPast: boolean; onRegister: () => void }) {
  const formattedDate = new Date(event.date).toLocaleDateString('en-GB', {
    weekday: 'short',
    day: '2-digit',
    month: 'short',
      ...
      <section className="section">
        <div className="container-custom">
          ...
          {visible.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-10">
              {visible.map((event) => (
                <EventCard key={event.slug} event={event} isPast={tab === 'past'} onRegister={() => setRegisterEvent(event)} />
              ))}
            </div>
          ) : (
            <div className="text-center py-20 text-gray-400">
