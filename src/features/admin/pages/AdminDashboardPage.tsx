...
function PendingMemberRow({
  ...
  const handle = async (action: 'approve' | 'reject') => {
    setBusy(true);
    if (action === 'approve') await adminDashboardApi.approveMember(member.id);
    else await adminDashboardApi.rejectMember(member.id);
    if (action === 'approve') onApprove(member.id);
    else onReject(member.id);
  };

  return (
    <div className="flex flex-col gap-3 rounded-2xl border border-accent-100 bg-accent-50/60 px-4 py-4 sm:flex-row sm:items-center sm:justify-between">
      ...
      <div className="flex gap-2 flex-shrink-0">
        ...
        >
          {busy ? <Icon icon="mdi:loading" className="w-4 h-4 animate-spin" /> : 'Approve'}
        </button>
        <button
          type="button"
          disabled={busy}
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        if (!cancelled) setError('Admin dashboard could not be loaded.');
      } finally {
      ...
      <section className="section bg-[radial-gradient(circle_at_top_left,_rgba(59,130,246,0.1),_transparent_30%),linear-gradient(180deg,_#f8fafc,_#ffffff)]">
        <div className="container-custom space-y-6">
          ...
          <section className="grid gap-6 xl:grid-cols-[1.3fr_0.7fr]">
            ...
            <div className="space-y-6">
              <SectionCard
                title={`Pending Approvals ${pending.length > 0 ? `(${pending.length})` : ''}`}
              >
                {pending.length === 0 ? (
                ...
                <div className="space-y-3">
                  {dashboard.recentMembers.map((m) => (
                    ...
                    >
                      ...
                      <span className="text-xs text-accent-400 flex-shrink-0">{m.joinedAt}</span>
                    </div>
                  ))}
                </div>
              </SectionCard>
