...
function StatCard({
  label, value, detail, icon, tone, loading,
}: {
  label: string; value: string; detail: string;
  icon: string; tone: StatTone; loading?: boolean;
}) {
  return (
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-3 text-2xl font-bold">
            {loading ? <span className="inline-block h-7 w-10 animate-pulse rounded-lg bg-white/20" /> : value}
          </p>
...
export function UserDashboardPage() {
  ...
  if (currentUser && !currentUser.residentialAddress)   profileTasks.push('Add your residential address');
  ...
  const communityUpdates = [
    { title: 'Mentorship circle opens next week',    subtitle: 'Join a cross-year mentoring group before seats fill up.',        href: '/about' },
    { title: 'Regional meetup planning has started', subtitle: 'See the latest planning note and volunteer opportunities.',       href: '/events' },
    { title: 'Community directory refresh underway', subtitle: 'Profiles with complete details will be featured more often.',     href: '/alumni/profiles' },
  ];
          ...
          <section className="grid gap-6 xl:grid-cols-[1.15fr_0.85fr]">
            ...
            <div className="space-y-6">
              ...
              <SectionCard
                title="Announcements"
                action={
                  <AppLink href="/announcements" className="text-sm font-semibold text-primary-600 hover:text-primary-700">
                    View all
                  </AppLink>
                }
              >
                {announcementsLoading ? <SectionSkeleton rows={3} /> : (
                  <div className="space-y-3">
                      ...
                      >
                        ...
                        <div className="min-w-0">
                          ...
                          {/* <p className="mt-1 text-sm text-accent-600">
                            {item.category} • {formatAnnouncementDate(item.date)}
                          </p> */}
                        </div>
                      </AppLink>
                    ))}
              ...
              >
                {eventsLoading ? <SectionSkeleton rows={3} /> : (
                  <div className="space-y-3">
                    {dashboardEvents.map((event) => (
                      ...
                      >
                        ...
                        <AppLink href={`/events/${event.slug}`} className="btn btn-primary btn-sm justify-center">
                          RSVP
                        </AppLink>
                      </div>
                    ))}
            ...
            <div className="space-y-6">
              ...
              <SectionCard title="Profile Completion">
                <div className="rounded-2xl border border-primary-100 bg-primary-50 p-4">
                  <div className="flex items-center justify-between gap-4">
                    <div>
                      <p className="text-sm font-medium text-primary-900">Profile progress</p>
                      <p className="mt-1 text-3xl font-bold text-primary-900">{profileCompletion}%</p>
                    </div>
                    <div className="rounded-2xl bg-white px-3 py-2 text-sm font-semibold text-primary-700 shadow-sm">
                  ...
                  <div className="mt-4 space-y-3">
                    {profileTasks.map((task) => (
                      <div key={task} className="flex items-center gap-3 text-sm text-accent-700">
                        <Icon icon="mdi:checkbox-blank-circle-outline" className="h-4 w-4 flex-shrink-0 text-primary-500" />
                        <span>{task}</span>
                      </div>
                    ))}
                  </div>
              ...
              <SectionCard
                title="Suggested Alumni"
                action={
                  <AppLink href="/alumni/profiles" className="text-sm font-semibold text-primary-600 hover:text-primary-700">
                    View directory
                  </AppLink>
                }
              >
                {alumniLoading ? <SectionSkeleton rows={2} /> : (
                  <div className="space-y-3">
                    {suggestedAlumni.map((alumnus) => (
                      ...
                      >
                        <div className="flex h-11 w-11 flex-shrink-0 items-center justify-center rounded-2xl bg-accent-100 font-semibold text-accent-700">
                          {alumnus.name.split(' ').slice(0, 2).map((p) => p[0]).join('')}
                        </div>
                        <div className="min-w-0">
